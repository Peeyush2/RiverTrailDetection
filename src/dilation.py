...
def dilatation(val):
    dilatation_size = 2#cv.getTrackbarPos(title_trackbar_kernel_size, title_dilation_window)
    dilation_shape = morph_shape(0)#cv.getTrackbarPos(title_trackbar_element_shape, title_dilation_window))
...
def erosion(val):
    ...
    erosion_dst = cv.morphologyEx(src, cv.MORPH_OPEN ,element)
